...
class CTemplate_CatalogVideoItem extends CCommonTask
{
    var $fields = NULL;
    ...
    function getText()
    {
        ...
        if (strtolower(get_class($r->directors)) == "ccollection")
        {
            ...
            $ret .= sprintf($ui->item("DIRECTOR_IS"), join($directors_buff, ", ")."\n");
            $was_flag = TRUE;
        }
        ...
        if (strtolower(get_class($r->producers)) == "ccollection")
        {
            ...
            if ($was_flag) $ret .= "<br>";
            $ret .= sprintf($ui->item("PRODUCED_BY"), join($prod_buff, ", ")."\n");
        ...
        if (strtolower(get_class($r->actors)) == "ccollection")
        {
            ...
            if ($was_flag) $ret .= "<br>";
            $ret .= sprintf($ui->item("VIDEO_ACTOR_IS"), join($actors_buff, ", ")."\n");
        ...
        if (strtolower(get_class($r->credits)) == "ccollection")
        {
            ...
            if ($was_flag) $ret .= "<br>";
            $ret .= sprintf($ui->item("VIDEO_CREDITS_IS"), join($credits_buff, ", ")."\n");
        ...
        //Added [15/04/2006 19:00] [Dain] vet_control_number output if current lang is finnish
        //
        ...
        {
            $ret .= '<br/>Vapautettu luokittelusta<br/>';
        }
...
return new CTemplate_CatalogVideoItem();
