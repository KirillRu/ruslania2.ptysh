...
class CTemplate_CatalogPereodicsBrowseItems extends CCommonTask
{
    var $fields = NULL;
    var $html = NULL;
    ...
    {
        if (!is_a($r, "CCatalog"))
            trigger_error("CTemplate_CatalogPereodicsBrowseItems::setData() accepts only php 'CCatalog' class as argument.", E_USER_ERROR);
        else
            $this->fields = &$r;
    ...
    function __getItemText(&$r, $need_separator)
    {
        ...
        {
            $item_table .=
              "<img src=\"/pic1/nophoto.gif\" border=0>";
        }
        else
        {
            $item_table .=
        ...
        {
            $item_table .=
              sprintf ($ui->item("TYPE_OF"),
              '<a href="'.$r->filter_type_url.'"'.
                'title="'.$ui->item("FILTER_PEREODICALS_TYPE").'" class="cprop">'.
                $r->type_name.
              '</a>'.
              '<br>');
        }
        ...
        if (!empty($r->issues_year))
        {
            ...
            $item_table .=
              sprintf($x_issues_in_year, $issues_year).", ";
        }
        ...
        {
            $item_table .= "<br>".sprintf($ui->item("COUNTRY_OF_ORIGIN"), $r->country_name);
        }
        
        if ($r->index != NULL)
        {
            $item_table .= "<br>".sprintf($ui->item("PERIOD_INDEX"), $r->index);
        }
        
        // NOTE: [03.03.2008 17:26][lex] ISSN output added
        if ($r->issn)
        {
            $item_table .= "<br>"."ISSN ".$r->issn;
        }
        ...
        $item_table .=
          ...
          '<a href="'.$r->category_url.'" '.
            'title="'.$ui->item("DISPLAY_SUBCATEGORY_ITEMS").'" '.
            'class="catlist">'.
            $r->category_title.
          '</a>';
...
*/
        ...
        if ($r->have_lookinside)
        {
            $item_table .=
              '<div class=mt5>'.
                '<a class="pointerhand" '.
                'onclick="window.open(\''.$r->lookinside_url.'\', \'_blank\', '.
                  '\'height=500, width=640, status=yes, toolbar=yes, '.
        ...
        // NOTE: reversing list to get biggest subscription period as default
        $r->months_list->reverse();
        $saveResult = $this->argv->getSaveResult
                      (
                        $this->argv->getSaveEntry
                        (
                          "cart_add_quantity",
                          Array($r->id => $r->months_list->item())
                        )
                      );
        $r->months_list->reverse();

        //
        // NOTE: [15.06.2005 14:17][den] create subscription price
        //       calculation block for item
        //
        ...
        $month_select_html =
                $this->html->createSingle_SELECT(
                        $selectStyle, $saveResult, $localizedMonths);
        ...
        $form_hidden_html =
                $this->html->createMulitple_INPUT(
                        "type=hidden", $saveResult);
        ...
        $item_table .= '</td></tr></form></table>';

        return $item_table;
    }
}

return new CTemplate_CatalogPereodicsBrowseItems();
