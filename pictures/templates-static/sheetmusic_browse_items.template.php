...
class CTemplate_CatalogSheetmusicBrowseItems extends CCommonTask
{
    var $fields = NULL;
    ...
    {
        if (!is_a($r, "CCatalog"))
            trigger_error("CTemplate_CatalogSheetmusicBrowseItems::setData() accepts only php 'CCatalog' class as argument.", E_USER_ERROR);
        else
            $this->fields = &$r;
    }
    ...
    function __getItemText(&$r, $need_separator)
    {
        ...
        if (strtolower(get_class($r->authors)) == "ccollection")
        {
            ...
            while($r->authors->next())
            {
                ...
                $authors_buff[] = 
                    "<a title=\"".$ui->item("FILTER_SHEETMUSIC_AUTHOR")."\" ".
        ...
        if (!empty($r->publisher_name))
        {
            $item_table .= sprintf(
                ...
                "title=\"".$ui->item("FILTER_SHEETMUSIC_PUBLISHER")."\" class=\"cprop\">".
                    $r->publisher_name.
                "</a> "
            );
        }
        ...
        if (!empty($r->series_id))
        {
            $item_table .= '<br>'.
                sprintf (
                    $ui->item("SERIES_IS"),
                    "<a href=\"".$r->series_url."\" ".
                    "title=\"".$ui->item("FILTER_SHEETMUSIC_SERIES")."\" class=\"cprop\">".
                        $r->series_name.
                    "</a> "
                );
        }
        ...
        if (!empty($r->pages))
        {
            ...
            if (!empty($r->binding_name)) $item_table .= ", ";
        }
        ...
        if ( !empty($r->binding_id) )
        {
            ...
            $item_table .= sprintf (
                ...
                "title=\"".$ui->item("FILTER_SHEETMUSIC_BINDING_TYPE")."\" class=\"cprop\">".
                    $r->binding_name.
                "</a> "
            );
        }
            ...
            $item_table .=
                "<div class=\"mt5\">".
                    ...
                    ">".
                        ...
                        "alt=\"".$this->ui->item("MSG_BTN_LOOK_INSIDE")."\" border=\"0\">".
                    "</a>".
                "</div>";
        }
...
return new CTemplate_CatalogSheetmusicBrowseItems();
